...
sorted_pairs: list[dict[str,JunctionBox|int]] = sorted(junction_box_pairs, key=lambda i: i["distance"])
for pair in sorted_pairs:
    ...
    if len(circuits) == 1:
        print(f"Last circuit pair: {pair["a"]} {pair["b"]}")
        print(f"Flag: {pair["a"].x * pair["b"].x}")
        break
